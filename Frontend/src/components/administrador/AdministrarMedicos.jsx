...
function AdministrarMedicos() {
  ...
  const editarMedico = async (e) => {
    ...
    try {
      const res = await fetch(
        `${import.meta.env.VITE_API_URL}/medicos/${form.id}`,
        {
          method: "PUT",
          headers: {
      ...
      if (data.ok) {
        ...
        Swal.fire(
          "Error",
          data.errores?.[0] || "No se pudo editar",
          "error"
        );
      }
    } catch {
  ...
  const eliminarMedico = async (medico) => {
    ...
    try {
      const res = await fetch(
        `${import.meta.env.VITE_API_URL}/medicos/${medico.Id_Medicos}`,
        {
          method: "DELETE",
        }
      );
      ...
      if (data.ok) {
        ...
        Swal.fire(
          "Error",
          data.errores?.[0] || "No se pudo eliminar",
          "error"
        );
      }
    } catch {
  ...
  return (
    <div className="admin-medicos-page">
      <div className="header">
        ...
        <p>
          Gestiona el equipo médico especializado de la Clínica del Bienestar
        </p>
      </div>
